    ...
    <div>
      ...
      {isOpen && (
        <div className="fixed top-0 left-0 w-full h-full bg-[rgba(0,0,0,0.6)] flex justify-center items-center z-[60]">
          <div
            className="flex flex-col bg-[#181818] w-[86%] max-w-[640px] mx-auto rounded-[6px] px-4 py-6 shadow-[6px_4px_4px_rgba(0,0,0,0.25)] text-white
            items-center space-y-8"
          >
            <div className="text-3xl font-bold">投稿を削除しますか？</div>
            <div className="flex items-center w-[40%] justify-between">
              <button onClick={() => handleDelete(id)} disabled={isSubmitting}>
                <span className="text-[#ff0000]">{isSubmitting ? "削除中..." : "削除"}</span>
              </button>
