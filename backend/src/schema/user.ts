...
const baseUserSchema = z.object({
    ...
    email : z.string()
        .min(1 , "メールアドレスは必須です。")
        .max(32 , "メールアドレスは32文字以内で入力してください。")
    .email("メールアドレスの形式が正しくありません。"),
    password : z.string()
        .min(8 , "パスワードは8文字以上にしてください。")
        .max(20 , "パスワードは20文字以内にしてください")
        .regex(/(?=.*[0-9])(?=.*[a-z])(?=.*[A-Z])/, "パスワードは数字・英小文字・英大文字をそれぞれ1文字以上使用してください"),
    icon : z.custom<File>()
        .refine((file) => file.size <= ALLOWED_IMAGE_SIZE , "アイコンは6MB以内にしてください")
        .refine((file) => ALLOWED_IMAGE_TYPE.includes(file.type) , "アイコンはjpeg,png,jpgのみアップロードできます。")
        .optional(),
})
