...
export const updatePostSchema = basePostSchema.extend({
  photo: z.custom<File>()
    .refine(file => !file || file.size <= ALLOWED_IMAGE_SIZE , { message : 'メニューの写真は6MB以内にしてください。' })
    .refine(file => !file || ALLOWED_IMAGE_TYPE.includes(file.type) , { message : 'メニューの写真はjpeg,png,jpgのみアップロードできます。' })
    .optional(),
});
